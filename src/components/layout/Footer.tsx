...
export function Footer() {
  return (
    <footer className="bg-[#070707] border-t border-gold/10">
      {/* Main footer */}
      <div className="max-w-7xl mx-auto px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
            ...
            <div className="flex gap-3">
              {socials.map(({ label, href, title }) => (
                <a
                  ...
                  aria-label={title}
                  title={title}
                  className="w-9 h-9 border border-gold/30 flex items-center justify-center text-gray-400 hover:text-gold hover:border-gold transition-all duration-200 text-xs font-bold tracking-wider"
                >
                  {label}
                </a>
              ))}
          ...
          {Object.entries(footerLinks).map(([title, links]) => (
            <div key={title}>
              <h4 className="text-white text-xs font-bold tracking-widest uppercase mb-6 pb-3 border-b border-gold/20">
                {title}
              </h4>
      ...
      <div className="border-t border-gold/10">
        <div className="max-w-7xl mx-auto px-6 lg:px-8 py-5 flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <p className="text-gray-500 text-xs flex items-center gap-1">
            Made with <Heart size={10} className="text-gold" fill="#C9A84C" /> for communities worldwide
          </p>
        </div>
      </div>
    </footer>
